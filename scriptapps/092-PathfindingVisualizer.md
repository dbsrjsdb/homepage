...
Create a focused, reliable browser-based pathfinding visualizer utility that fulfills the listed project objective: Draw obstacles and compare BFS, DFS, Dijkstra and A*.
...
Users need a straightforward way to draw obstacles and compare BFS, DFS, Dijkstra and A*. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core pathfinding visualizer workflow described above.
...
* [x] The core pathfinding visualizer workflow works as described.
...
* Implemented editable 24×14 grid search with breadth-first search, depth-first search, Dijkstra, and A* options.
* Added animated visited/path states, wall editing, speed control, clear behavior, metrics, no-route feedback, accessible grid cells, and responsive layout.
...
* Start and goal cells are fixed for consistent comparisons; walls are user-editable.
* Search state is held in memory and is not persisted.
