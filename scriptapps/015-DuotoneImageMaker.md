...
Create a focused, reliable browser-based duotone image maker utility that fulfills the listed project objective: Apply customizable two-color effects to images.
...
Users need a straightforward way to apply customizable two-color effects to images. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core duotone image maker workflow described above.
...
* [x] The core duotone image maker workflow works as described.
...
* Added local image upload and customizable shadow/highlight color mapping.
* Added contrast and inversion controls, live preview, PNG download, reset behavior, validation, and live status feedback.
...
* Verified static loading, duotone controls, responsive viewport behavior, and accessibility markers in Chromium.
...
* Downsample large images to a maximum 720 pixels on the longest side and map luminance between the two selected colors.
