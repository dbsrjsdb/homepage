...
Create a focused, reliable browser-based color palette extractor utility that fulfills the listed project objective: Upload an image and extract its dominant colors.
...
Users need a straightforward way to upload an image and extract its dominant colors. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core color palette extractor workflow described above.
...
* [x] The core color palette extractor workflow works as described.
...
* Added local image upload, preview, and dominant-color extraction with 3–10 color controls.
* Added color cards with hex values, sample percentages, individual copy actions, CSS palette copy, and text download.
* Added reset behavior, validation, live status feedback, responsive layouts, visible focus states, and semantic labels.
* Added development and usage documentation.
* Verified static loading, palette controls, responsive viewport behavior, and accessibility markers in Chromium.
...
* Downsample to a maximum 90×90 sample canvas and quantize RGB channels into 32-value buckets for a lightweight, deterministic palette.
