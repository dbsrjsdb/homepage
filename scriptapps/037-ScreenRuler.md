...
Create a focused, reliable browser-based screen ruler utility that fulfills the listed project objective: Calibratable browser ruler for measuring objects on screen.
...
Users need a straightforward way to calibratable browser ruler for measuring objects on screen. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core screen ruler workflow described above.
...
* [x] The core screen ruler workflow works as described.
...
* Selected a dependency-free HTML, CSS, and JavaScript implementation using CSS-rendered ruler ticks and browser viewport measurements.
* Implemented pixel length, unit conversion, horizontal/vertical orientation, calibration against physical references, tick labels, reset calibration, and input validation.
* Added responsive styling, semantic labels, visible focus states, live status output, and documentation.
* Validated JavaScript syntax, whitespace, static loading, calibrated measurement output, ruler controls, accessibility attributes, and mobile rendering.
