...
Create a focused, reliable browser-based sorting algorithm race utility that fulfills the listed project objective: Animate several sorting algorithms simultaneously.
...
Users need a straightforward way to animate several sorting algorithms simultaneously. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core sorting algorithm race workflow described above.
...
* [x] The core sorting algorithm race workflow works as described.
...
* Implemented synchronized bubble, selection, and insertion sort generators operating on the same shuffled values.
* Added animated bar charts, comparison/swap metrics, array-size and speed controls, new-data generation, accessible status feedback, and responsive cards.
...
* The visualizer uses bounded arrays of 18–72 values and educational operation counts.
* Race state is held in memory and is not persisted.
