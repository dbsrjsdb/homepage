...
Create a focused, reliable browser-based kanban board utility that fulfills the listed project objective: Drag cards between columns, persisted with localStorage.
...
Users need a straightforward way to drag cards between columns, persisted with localStorage. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core kanban board workflow described above.
...
* [x] The core kanban board workflow works as described.
...
* Implemented three columns, task creation, drag movement, keyboard-focusable move buttons, deletion, clear/restore actions, counts, and persistence.
* Added safe task rendering, responsive styling, semantic labels, visible focus states, live status output, and documentation.
* Validated JavaScript syntax, whitespace, static loading, seeded board columns/cards, persistence wiring, accessibility controls, and mobile rendering.
