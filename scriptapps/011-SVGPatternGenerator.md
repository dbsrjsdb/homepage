...
Create a focused, reliable browser-based svg pattern generator utility that fulfills the listed project objective: Generate customizable geometric backgrounds and export SVG.
...
Users need a straightforward way to generate customizable geometric backgrounds and export SVG. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core svg pattern generator workflow described above.
...
* [x] The core svg pattern generator workflow works as described.
...
* Added grid, dots, diagonal stripe, and diamond SVG pattern styles.
* Added background/shape colors, tile size, line weight, rotation, and opacity controls.
* Added live SVG preview, markup display, copy, download, reset behavior, and live status feedback.
...
* Verified static loading, pattern controls, responsive viewport behavior, and accessibility markers in Chromium.
...
* Generate a self-contained SVG using a `pattern` definition and standard geometric primitives.
