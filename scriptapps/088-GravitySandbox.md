...
Create a focused, reliable browser-based gravity sandbox utility that fulfills the listed project objective: Place planets and watch an N-body simulation evolve.
...
Users need a straightforward way to place planets and watch an N-body simulation evolve. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core gravity sandbox workflow described above.
...
* [x] The core gravity sandbox workflow works as described.
...
* Implemented an interactive N-body Canvas simulation with a seeded three-body starting system and click-to-add planets.
* Added gravity/time-scale sliders, orbital trails, pause/reset controls, live body count, accessible canvas input, and status feedback.
...
* The simulation uses an O(n²) force pass and a softening term for close approaches.
* Bodies remain in memory and may leave the visible canvas during unstable trajectories.
