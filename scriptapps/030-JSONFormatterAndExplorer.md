...
Create a focused, reliable browser-based json formatter & explorer utility that fulfills the listed project objective: Validate, prettify, minify and collapse JSON trees.
...
Users need a straightforward way to validate, prettify, minify and collapse JSON trees. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core json formatter & explorer workflow described above.
...
* [x] The core json formatter & explorer workflow works as described.
...
* Selected a dependency-free HTML, CSS, and JavaScript implementation using native `JSON.parse` and `JSON.stringify`.
* Implemented validation, prettification, minification, formatted output, a collapsible native `details` tree, expand/collapse controls, copy, and clear actions.
* Added invalid-input feedback, responsive styling, semantic labels, visible focus states, live status output, and documentation.
* Validated JavaScript syntax, whitespace, static loading, representative formatted JSON and nested tree output, accessibility attributes, and mobile rendering.
