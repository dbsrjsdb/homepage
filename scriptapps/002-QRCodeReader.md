...
Create a focused, reliable browser-based qr code reader utility that fulfills the listed project objective: Drop an image containing a QR code and decode it locally.
...
Users need a straightforward way to drop an image containing a QR code and decode it locally. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core qr code reader workflow described above.
...
* [x] The core qr code reader workflow works as described.
...
* Selected a plain HTML, CSS, and JavaScript implementation with jsQR 1.4.0 from jsDelivr.
* Added image file selection and drag-and-drop QR decoding entirely in the browser.
* Added local image preview, decoded text output, conditional link opening, copy action, and reset behavior.
* Added clear feedback for empty state, invalid file types, oversized files, unreadable images, missing QR codes, and decoder-load failures.
* Added responsive layouts, visible focus states, semantic labels, live status messaging, and output labelling.
* Verified the workflow with a generated QR image, invalid input, reset, accessibility, and mobile checks in headless Chromium.
