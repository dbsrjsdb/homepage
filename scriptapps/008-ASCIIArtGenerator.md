...
Create a focused, reliable browser-based ascii art generator utility that fulfills the listed project objective: Convert an uploaded image into adjustable ASCII artwork.
...
Users need a straightforward way to convert an uploaded image into adjustable ASCII artwork. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core ascii art generator workflow described above.
...
* [x] The core ascii art generator workflow works as described.
...
* Selected a plain HTML, CSS, and JavaScript implementation with Canvas sampling.
* Added image upload and local conversion to adjustable monospaced ASCII output.
* Added character width, simple/detailed/block/custom character sets, contrast, and inversion controls.
* Added original preview, copy, text download, reset behavior, validation, and live status feedback.
* Added responsive layouts, visible focus states, semantic labels, and labelled ASCII output.
* Added development and usage documentation.
* Verified static loading, generator controls, responsive viewport behavior, and accessibility markers in Chromium.
...
* Sample images to a character grid through an offscreen Canvas and compensate for monospace character proportions when calculating rows.
