...
Create a focused, reliable browser-based gradient studio utility that fulfills the listed project objective: Visually build linear/radial/conic CSS gradients and copy the CSS.
...
Users need a straightforward way to visually build linear/radial/conic CSS gradients and copy the CSS. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core gradient studio workflow described above.
...
* [x] The core gradient studio workflow works as described.
...
* Added linear, radial, and conic gradient types with live preview.
* Added editable color stops, positions, add/remove controls, and angle settings.
* Added generated CSS display, clipboard copy, stylesheet download, reset behavior, and live status feedback.
* Added responsive layouts, visible focus states, semantic labels, and accessible stop controls.
* Added development and usage documentation.
* Verified static loading, gradient controls, responsive viewport behavior, and accessibility markers in Chromium.
...
* Keep gradient state as a small in-memory list of color stops and generate standard CSS gradient functions directly.
