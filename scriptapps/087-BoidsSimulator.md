...
Create a focused, reliable browser-based boids simulator utility that fulfills the listed project objective: Interactive flocking simulation with cohesion/separation/alignment controls.
...
Users need a straightforward way to interactive flocking simulation with cohesion/separation/alignment controls. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core boids simulator workflow described above.
...
* [x] The core boids simulator workflow works as described.
...
* Implemented live flocking with alignment, cohesion, separation, edge wrapping, and pointer attraction rules.
* Added flock-size control, pause/reset actions, live count/status feedback, accessible canvas controls, and responsive layout.
...
* The simulator uses a bounded flock of 20–100 boids and an O(n²) neighborhood pass.
* Flock state is kept in memory and is not persisted.
