...
Create a focused, reliable browser-based jwt inspector utility that fulfills the listed project objective: Decode JWT header/payload locally and display expiration information.
...
Users need a straightforward way to decode JWT header/payload locally and display expiration information. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core jwt inspector workflow described above.
...
* [x] The core jwt inspector workflow works as described.
...
* Implemented local Base64URL/JSON JWT decoding with formatted header/payload output, exp/iat timing checks, sample/clear/copy controls, and invalid-token feedback.
* Added explicit signature-verification boundary messaging, responsive layout, semantic labels, and no token persistence.
...
* This tool decodes but does not verify JWT signatures or make authorization decisions.
