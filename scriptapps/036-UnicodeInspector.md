...
Create a focused, reliable browser-based unicode inspector utility that fulfills the listed project objective: Type characters and inspect code points, UTF-8 bytes and names.
...
Users need a straightforward way to type characters and inspect code points, UTF-8 bytes and names. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core unicode inspector workflow described above.
...
* [x] The core unicode inspector workflow works as described.
...
* Selected a dependency-free HTML, CSS, and JavaScript implementation using `Array.from` and `TextEncoder`.
* Implemented code point, hexadecimal, decimal, UTF-8 byte, and readable-name columns, sample/copy/clear actions, counts, and empty-input feedback.
* Added safe table rendering, responsive styling, semantic labels, visible focus states, live status output, and documentation.
* Validated JavaScript syntax, whitespace, static loading, representative Unicode code points and UTF-8 bytes, accessibility attributes, and mobile rendering.
