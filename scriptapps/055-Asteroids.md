...
Create a focused, reliable browser-based asteroids utility that fulfills the listed project objective: Vector-style spaceship game using Canvas.
...
Users need a straightforward way to vector-style spaceship game using Canvas. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core asteroids workflow described above.
...
* [x] The core asteroids workflow works as described.
...
* Implemented a dependency-free vector-style Canvas game with ship rotation/thrust, firing, wrapping, asteroid splitting, waves, lives, scoring, pause, restart, and touch controls.
* Added responsive styling, semantic control labels, live status feedback, and browser-only state.
...
* Gameplay depends on a browser with Canvas support; no backend or remote state is required.
