...
Create a focused, reliable browser-based one-line puzzle utility that fulfills the listed project objective: Connect every node exactly once without lifting the pointer.
...
Users need a straightforward way to connect every node exactly once without lifting the pointer. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core one-line puzzle workflow described above.
...
* [x] The core one-line puzzle workflow works as described.
...
* Implemented three connect-the-nodes layouts with pointer drag sequencing, progress tracking, wrong-node feedback, reset, hint highlighting, and completion persistence.
* Added responsive SVG rendering, semantic node labels, keyboard focus styling, and live status feedback.
...
* The supported layouts use predefined routes; pointer and keyboard input are handled locally.
