...
Create a focused, reliable browser-based image resizer utility that fulfills the listed project objective: Drop an image, set dimensions/percentage, download the resized result.
...
Users need a straightforward way to drop an image, set dimensions/percentage, download the resized result. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core image resizer workflow described above.
...
* [x] The core image resizer workflow works as described.
...
* Added exact-dimension resizing with optional aspect-ratio preservation.
* Added percentage resizing from 1% to 1000%.
* Added PNG, JPEG, and WebP downloads, reset behavior, validation, and live status feedback.
...
* Verified image upload, dimension resizing, percentage resizing, reset behavior, static loading, responsive viewports, and accessibility markers in Chromium.
...
* Use a 20 MB input limit and clamp output dimensions to 10,000 pixels per side to avoid unreasonable browser memory use.
