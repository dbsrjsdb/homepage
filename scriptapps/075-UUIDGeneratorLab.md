...
Create a focused, reliable browser-based uuid generator/lab utility that fulfills the listed project objective: Generate batches of UUIDs and inspect their structure.
...
Users need a straightforward way to generate batches of UUIDs and inspect their structure. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core uuid generator/lab workflow described above.
...
* [x] The core uuid generator/lab workflow works as described.
...
* Implemented local version 4 UUID batch generation with quantity, case, and hyphen controls.
* Added per-UUID structure inspection, batch and individual copy actions, clear/reset behavior, status feedback, responsive layout, and accessible controls.
* Used `crypto.randomUUID()` with a `crypto.getRandomValues()` fallback for browsers without the convenience method.
...
* UUID generation requires a browser crypto API; older or restricted browsers may not support the fallback.
