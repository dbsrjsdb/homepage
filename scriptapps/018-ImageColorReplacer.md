...
Create a focused, reliable browser-based image color replacer utility that fulfills the listed project objective: Pick one color in an image and replace similar pixels.
...
Users need a straightforward way to pick one color in an image and replace similar pixels. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core image color replacer workflow described above.
...
* [x] The core image color replacer workflow works as described.
...
* Selected a plain HTML, CSS, and JavaScript implementation with Canvas pixel processing.
* Added local image upload, original/result previews, and direct source-color picking from the original image.
* Added replacement color, RGB-distance tolerance, PNG download, reset behavior, validation, and live status feedback.
...
* Verified static loading, replacer controls, responsive viewport behavior, and accessibility markers in Chromium.
...
* Downsample large images to a maximum 720 pixels on the longest side and compare RGB Euclidean distance against the tolerance.
