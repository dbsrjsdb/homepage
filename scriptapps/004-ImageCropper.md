...
Create a focused, reliable browser-based image cropper utility that fulfills the listed project objective: Interactive crop box with aspect-ratio presets.
...
Users need a straightforward way to interactive crop box with aspect-ratio presets. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core image cropper workflow described above.
...
* [x] The core image cropper workflow works as described.
...
* Added a movable, resizable crop frame with corner handles.
* Added free, 1:1, 4:3, 3:4, 16:9, and 9:16 aspect-ratio presets.
* Added PNG, JPEG, and WebP crop export, reset behavior, validation, and live status feedback.
...
* Verified image upload, bounded aspect-ratio cropping, Canvas export, reset behavior, static loading, responsive viewports, and accessibility markers in Chromium.
...
* Use a 20 MB input limit and keep all source and crop data in the browser.
* Size the crop stage to the displayed image so the selection cannot extend into blank editor space.
* Use Canvas `toBlob` for downloads and clamp crop selections to the image bounds.
