...
Create a focused, reliable browser-based music sequencer utility that fulfills the listed project objective: 16-step drum/synth sequencer using Web Audio.
...
Users need a straightforward way to 16-step drum/synth sequencer using Web Audio. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core music sequencer workflow described above.
...
* [x] The core music sequencer workflow works as described.
...
* Implemented a 16-step four-voice sequencer for kick, snare, hi-hat, and bass using synthesized Web Audio tones.
* Added accessible step toggles, tempo control, play/stop, randomize, clear, current-step highlighting, and local status feedback.
...
* Playback requires a browser with Web Audio API support and a user gesture to satisfy autoplay policies.
* Patterns are held in memory and are not persisted.
