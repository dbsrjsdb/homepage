...
Create a focused, reliable browser-based photo mosaic generator utility that fulfills the listed project objective: Reconstruct an image from colored tiles or emoji.
...
Users need a straightforward way to reconstruct an image from colored tiles or emoji. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core photo mosaic generator workflow described above.
...
* [x] The core photo mosaic generator workflow works as described.
...
* Added local image upload and average-color mosaic generation.
* Added adjustable tile columns, tile gap, color-tile mode, and emoji mode.
* Added mosaic preview, PNG download, reset behavior, validation, live status feedback, and labelled output.
* Added responsive layouts, visible focus states, and semantic controls.
* Added development and usage documentation.
* Verified static loading, mosaic controls, responsive viewport behavior, and accessibility markers in Chromium.
...
* Downsample source images to the selected tile grid through an offscreen Canvas and render the result at a readable fixed tile size.
