...
Create a focused, reliable browser-based base64 workbench utility that fulfills the listed project objective: Encode/decode text, images and files.
...
Users need a straightforward way to encode/decode text, images and files. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core base64 workbench workflow described above.
...
* [x] The core base64 workbench workflow works as described.
...
* Selected a dependency-free HTML, CSS, and JavaScript implementation using UTF-8 encoders and browser Base64 primitives.
* Implemented text/file encode mode, Base64 decode mode, Unicode-safe text handling, byte downloads, copy, clear, size reporting, and invalid-input feedback.
* Added responsive styling, semantic labels, visible focus states, live status output, and documentation.
* Validated JavaScript syntax, whitespace, static loading, Unicode encoding status and byte counts, mode controls, accessibility attributes, and mobile rendering.
