...
Create a focused, reliable browser-based random picker utility that fulfills the listed project objective: Paste names/items and perform animated fair selections.
...
Users need a straightforward way to paste names/items and perform animated fair selections. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core random picker workflow described above.
...
* [x] The core random picker workflow works as described.
...
* Selected a dependency-free HTML, CSS, and JavaScript implementation using cryptographic random index selection.
* Implemented one-item-per-line input, animated selection, optional removal of picked items, sample/reset/clear-history actions, count feedback, and validation.
* Added safe history rendering, responsive styling, semantic labels, visible focus states, live status output, and documentation.
* Validated JavaScript syntax, whitespace, static loading, seeded item list and controls, randomization wiring, accessibility attributes, and mobile rendering.
