...
Create a focused, reliable browser-based infinite minesweeper utility that fulfills the listed project objective: Dynamically expand the minefield as the player explores.
...
Users need a straightforward way to dynamically expand the minefield as the player explores. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core infinite minesweeper workflow described above.
...
* [x] The core infinite minesweeper workflow works as described.
...
* Implemented a deterministic procedural minefield with an expandable 13×11 viewport, reveal/flag interactions, safe origin, panning, recentering, new-world reset, and mine feedback.
* Added responsive grid rendering, accessible cell labels, keyboard/button navigation, and local-only state.
...
* The viewport is finite at any moment; panning reveals the effectively unbounded procedural world.
