...
Create a focused, reliable browser-based url inspector utility that fulfills the listed project objective: Break a URL into protocol, host, path, query parameters and fragment.
...
Users need a straightforward way to break a URL into protocol, host, path, query parameters and fragment. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core url inspector workflow described above.
...
* [x] The core url inspector workflow works as described.
...
* Selected a dependency-free HTML, CSS, and JavaScript implementation using native `URL` and `URLSearchParams` APIs.
* Implemented protocol, host, port, path, username, fragment, normalized URL, query parameter table, sample, copy, clear, and invalid-URL feedback.
* Added safe query rendering, responsive styling, semantic labels, visible focus states, live status output, and documentation.
* Validated JavaScript syntax, whitespace, static loading, representative URL components and query output, accessibility attributes, and mobile rendering.
