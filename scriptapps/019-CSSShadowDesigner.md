...
Create a focused, reliable browser-based css shadow designer utility that fulfills the listed project objective: Interactive box/text shadow editor with live preview.
...
Users need a straightforward way to interactive box/text shadow editor with live preview. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core css shadow designer workflow described above.
...
* [x] The core css shadow designer workflow works as described.
...
* Added box-shadow and text-shadow target modes.
* Added horizontal/vertical offsets, blur, spread, opacity, shadow color, and preview background controls.
* Added live preview, CSS declaration output, copy, reset behavior, and live status feedback.
* Added responsive layouts, visible focus states, and semantic controls.
* Added development and usage documentation.
* Verified static loading, shadow controls, responsive viewport behavior, and accessibility markers in Chromium.
...
* Generate rgba shadow declarations from separate color and opacity controls so the CSS output is directly reusable.
