...
Create a focused, reliable browser-based viewport tester utility that fulfills the listed project objective: Preview an HTML snippet at several simulated device dimensions.
...
Users need a straightforward way to preview an HTML snippet at several simulated device dimensions. The project should provide a clear interface and immediate local feedback without unnecessary setup or data transfer.

## Requirements

* Implement the core viewport tester workflow described above.
...
* [x] The core viewport tester workflow works as described.
...
* Implemented a sandboxed HTML snippet preview with phone, tablet, desktop, and bounded custom viewport dimensions.
* Added live local updates, active breakpoint labels, accessible controls, status feedback, and responsive preview chrome.
...
* Preview scripts are disabled by the sandbox for safety; markup and inline styles remain available.
* External assets referenced by a snippet may be unavailable depending on browser/network policy.
